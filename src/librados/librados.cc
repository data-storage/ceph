    ...
    rados_ioctx_t io, uint32_t n, uint32_t m, rados_list_ctx_t *listh)
{
  ...
  */
  ...
  for (int N = 0; N < m; ++N) {
    //uint32_t pg_counter = (N * m) / pgs_per_worker;
    // pg_counter goes into the AAA bits
    ...
    //uint32_t pg_mask = (((m - 1) * m) / pgs_per_worker) << ppw_bits;
    ...
    uint32_t pg_counter = (N * (pg_pow2_count - 1)) / (m - 1);
    ...
    uint32_t hash_split_size = (0x7fffffff / pg_split_denom);
    ...
    ldout(ctx->client->cct, 10) << "N=" << N << std::hex << "hash_low = 0x" << hash_low << dendl;
    ldout(ctx->client->cct, 10) << "N=" << N << std::hex << "hash_high = 0x" << hash_high << dendl;
