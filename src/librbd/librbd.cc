...
int queue_aio_read(librbd::ImageCtx *ictx, uint64_t off, size_t len,
                   char *buf, bufferlist *pbl, librbd::AioCompletion *c,
                   int op_flags) {
  // prevent overflow of the 'bytes read' return code (legacy)
...
namespace librbd {
  ...
  */
		      ...
		      RBD::AioCompletion *c)
  {
    ...
    int r = queue_aio_read(ictx, off, len, NULL, &bl, get_aio_completion(c), 0);
			...
			RBD::AioCompletion *c, int op_flags)
  {
    ...
    ldout(ictx->cct, 10) << "Image::aio_read() buf=" << (void *)bl.c_str() << "~"
			 << (void *)(bl.c_str() + len - 1) << dendl;
    int r = queue_aio_read(ictx, off, len, NULL, &bl, get_aio_completion(c),
                           op_flags);
    tracepoint(librbd, aio_read_exit, r);
    return r;
  }
			    ...
			    char *buf, rbd_completion_t c)
{
  ...
  int r = queue_aio_read(ictx, off, len, buf, NULL, get_aio_completion(comp),
                         0);
  tracepoint(librbd, aio_read_exit, r);
  return r;
}
